...
function Actions:SlowLoop()
    if self.slowLoopRunning then
        return
    end

    self.slowLoopRunning = true
    CreateThread(function()
        ...
        self.slowLoopRunning = false
    end)
end

function Actions:PedLoop()
    if self.pedLoopRunning then
        return
    end

    self.pedLoopRunning = true
    CreateThread(function()
        while ESX.PlayerLoaded do
            self:TrackPed()
            Wait(250)
        end
        self.pedLoopRunning = false
    end)
end
